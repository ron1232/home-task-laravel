...
editForms.forEach(function (form) {
  form.addEventListener("submit", function (e) {
    ...
    axios__WEBPACK_IMPORTED_MODULE_2___default().put(form.action, _objectSpread({}, formData)).then(function () {
      window.location.reload();
    })["catch"](function (error) {
      if (error.response.data.errors) {
        if (error.response.data.errors.name) form.elements["edit-name"].classList.add("error");
        if (error.response.data.errors.iso) form.elements["edit-iso"].classList.add("error");
      }
    });
  });
});
